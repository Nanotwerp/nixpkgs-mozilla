...
let
  ...
  arch = if self.stdenv.system == "i686-linux" then "linux-i686" else "linux-x86_64";

  yearOf = with super.lib; yyyymmddhhmmss: head (splitString "-" yyyymmddhhmmss);
  monthOf = with super.lib; yyyymmddhhmmss: head (tail (splitString "-" yyyymmddhhmmss));
  ...
  extractSha512Sum =
    ...
    if builtins ? split then
      substring 0 128 (
        head (
          super.lib.filter (s: isString s && substring 128 (stringLength s) s == "  ${file}") (
            split "\n" sha512sums
          )
        )
      )
          ...
          if timestamp == null then
            let
              buildhubJSON =
                ...
                fromJSON (
                  readFile (
                    fetchurl "https://download.cdn.mozilla.net/pub/firefox/nightly/latest-mozilla-central/firefox-${version}.en-US.${system}.buildhub.json"
                  )
                );
            in
            builtins.replaceStrings [ "/${file}" ] [ "" ] buildhubJSON.download.url
          else
            "https://download.cdn.mozilla.net/pub/firefox/nightly/${yearOf timestamp}/${monthOf timestamp}/${timestamp}-mozilla-central";
        ...
        postFetch = ''
          : # Authenticity Check (${
            verifyFileAuthenticity {
              ...
              chksum = builtins.fetchurl {
                url = info.chksum;
                sha256 = info.chksumSha256;
              };
              chksumSig = builtins.fetchurl {
                url = info.chksumSig;
                sha256 = info.chksumSigSha256;
              };
            }
          })
        '';
...

{
  ...
  devEnv = (super.shell or { }) // {
    gecko = super.callPackage ./pkgs/gecko {
      ...
      nodejs =
        if builtins.compareVersions self.nodejs.name "nodejs-8.11.3" < 0 then
          self.nodejs-8_x
        else
          self.nodejs;

      rust-cbindgen =
        if !(self ? "rust-cbindgen") then
          self.rust-cbindgen-latest
        else if
          builtins.compareVersions self.rust-cbindgen.version self.rust-cbindgen-latest.version < 0
        then
          self.rust-cbindgen-latest
        else
          self.rust-cbindgen;
